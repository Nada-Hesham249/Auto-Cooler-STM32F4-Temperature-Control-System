...
static uint32 Timer_BaseAddresses[NUM_OF_TIMERS] = {TIM2_BASE_ADDR, TIM3_BASE_ADDR, TIM4_BASE_ADDR, TIM5_BASE_ADDR};   //address map
...
void Timer_Init(uint8 TimerId, uint16 Prescaler, uint16 AutoReload) {
    ...
    SET_BIT(timer->EGR, EGR_UG); // Force update to load PSC & ARR   to put the ARR and Ps values in buffer registers that Counter actually work with
...
void Timer_DelayMs(uint8 TimerId, uint32 DelayMs) {
    ...
    timer->PSC = 15999U / 3;    // actually 15999+1
    timer->ARR = (uint16) (DelayMs - 1);
    timer->CNT = 0;

    SET_BIT(timer->EGR, EGR_UG); // Load shadow registers  to put the ARR and Ps values in buffer registers that Counter actually work with
    timer->SR = 0; // Clear UIF caused by UG // clear status reg

    SET_BIT(timer->CR1, CR1_OPM); // One-pulse mode
    SET_BIT(timer->CR1, CR1_CEN); // Start counting cen ->enable
...
void Timer_DelayMsAsync(uint8 TimerId, uint32 DelayMs, TimerCallback Callback) {
    ...
    SET_BIT(timer->DIER, DIER_UIE); // Enable update interrupt from timer side
    Nvic_EnableIrq(irqNum);   // Enable update interrupt from NVIC side
...
static void Timer_HandleIrq(uint8 index) {
    ...
    if (READ_BIT(timer->SR, SR_UIF)) {     // to check if that from update event or others like input capture mode
        timer->SR = 0; // Clear UIF
