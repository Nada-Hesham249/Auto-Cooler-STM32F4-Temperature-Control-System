...
/* ================== Callback ================== */
void App_AdcCallback(uint16 result)
{
        App_LatestAdcValue = result;
...
void App_Init(void)
{
  ...
  //gpio init
   ...
   //lcd init
    ...
    Adc_ConfigSingleChannel_OneShot(APP_LM35_ADC_CHANNEL);
...
void App_Update(void)
{
    ...
    {
        /* ===== Critical Section to protect shared ADC data ===== */
        Nvic_DisableIrq(ADC_IRQ_NUMBER);
        local_adc_value = App_LatestAdcValue;
        App_NewSampleFlag = 0U;
        Nvic_EnableIrq(ADC_IRQ_NUMBER);


        /* ===== Convert ADC to temperature (fixed-point) ===== */
        float temperature_f=((float)(local_adc_value * 3300UL)) / 40960.0f;
        uint16 temperature_tenths_c =(uint16)(temperature_f*10);
        ...
        {
            Pwm_SetDutyPercent(TIMER3,PWM_CHANNEL_1,current_output.fan_speed);
        ...
        {
            Gpio_WritePin(APP_ALARM_PORT,APP_ALARM_PIN,current_output.overheat_flag);
        ...
        {
            Display_Update(temperature_tenths_c,current_output.fan_speed,current_output.overheat_flag);
        }
    }
}
